...
4. вывод списка транзакций по условию(code=USD), перечисление типов операций, генерация номеров карт-generators
5. создание в data файла mylog.txt с заполнением удачным результатом и/или ошибки при выполнении функции при заданном параметре filename-decorators
6. считывание json-файла и возврат списка транзакций-utils
7. ссылаясь на результат выполнения utils и обращаясь к API-ресурсу выводит сумму транзакций с конвертацией в рубли, если были в валюте-external_api
8. для модулей masks и utils так же добавлено логирование в папку logs
9. чтение csv/excel-файла и выводе их в виде списка без пустых строк-tables_reader
* Модуль main в корне проекта принимает значения и выводит результаты, прогоняя их по модулям из src, кроме:
- decorators(если был задан файл для записи)
- логирование(хода выполнения, возникшие ошибки)
* Директория tests применяется для проверок вхождения данных и их вывода после обработки, а так же отчет с покрытием кода в процентах, но не тестирует логирование
## Пример
Отображение вывода первых вхождений модуля main.py в активном окружении с использованной командой
